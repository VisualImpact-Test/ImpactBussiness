var Articulo = {
	...
	load: function () {
		...
		$(document).on('click', '#btn-registrarArticulo', function () {
			...
			let jsonString = { 'data': '' };
			let config = { 'url': Articulo.url + 'formularioRegistroArticulo', 'data': jsonString };

			$.when(Fn.ajax(config)).then((a) => {
				if (a.data.existe == 0) {
					Articulo.articulosLogistica = a.data.articulosLogistica;
				}
				...
				fn[1] = 'Fn.showConfirm({ idForm: "formRegistroArticulos", fn: "Articulo.registrarArticulo()", content: "¿Esta seguro de registrar el articulo?" });';
		...
		$(document).on('click', '.btn-actualizarArticulo', function () {
			...
			let config = { 'url': Articulo.url + 'formularioActualizacionArticulo', 'data': jsonString };
			...
			$.when(Fn.ajax(config)).then((a) => {
				...
				fn[0] = 'Fn.showModal({ id:' + modalId + ',show:false });';
				btn[0] = { title: 'Cerrar', fn: fn[0] };
				fn[1] = 'Fn.showConfirm({ idForm: "formActualizacionArticulos", fn: "Articulo.actualizarArticulo()", content: "¿Esta seguro de actualizar el articulo?" });';
		...
		$(document).on('click', '.btn-estadoArticulo', function () {
			...
			let jsonString = { 'data': JSON.stringify(data) };
			let config = { 'url': Articulo.url + 'actualizarEstadoArticulo', 'data': jsonString };
	...
	registrarArticulo: function () {
		...
		let url = Articulo.url + "registrarArticulo";
	...
	actualizarArticulo: function () {
		...
		let config = { 'url': Articulo.url + 'actualizarArticulo', 'data': jsonString };
	...
	actualizarAutocomplete: function () {
		$("#equivalente").autocomplete({
			source: Articulo.articulosLogistica[1],
			minLength: 0,
