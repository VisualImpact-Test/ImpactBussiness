...
class PagosGenerados extends MY_Controller
{
	...
	public function formularioRegistrarPago()
	{
		...
		$dataParaVista['metodoPago'] = $this->model->ObtenerDatosMetodoPago($post)['query']->result_array();
	...
	public function formularioRegistrarNuevoPago()
	{
		$result = $this->result;
		$post = json_decode($this->input->post('data'), true);
		$dataParaVista = [];
	...
	//	echo $this->db->last_query();exit();
		...
		$result['result'] = 1;
		$result['msg']['title'] = 'Registrar Nuevo Pago';
		$result['data']['html'] = $this->load->view("modulos/Finanzas/PagosGenerados/formularioRegistroNuevoPago", $dataParaVista, true);

		echo json_encode($result);
	}
	

	public function registrarPagoLibre()
	{
		$result = $this->result;
		$post = json_decode($this->input->post('data'), true);
		$this->db->trans_begin();
		...
		$archivo = [
			...
			'type' => $post['cuentaPrincipalPagoFile-type'],
			'carpeta' => 'FinanzasComprobantes',
			'nombreUnico' => uniqid()
		];
		$archivoName = $this->saveFileWasabi($archivo);
		...
		$insertFactura[] = [
			...
			'idTipoComprobante' => $post['tipoComprobante'],
			'numeroComprobante' => $post['numeroComprobante'],
			'montoPagado' => $post['monto_M'],
			'idCentroCosto' => $post['centroCostoForm'],
			'idCuenta' => $post['cuentaForm'],
			'porcentajeDetraccion' => $post['porcentajeDetraccion_M'],
			'montoDetraccion' => $post['montoDetraccion_M'],
			'idEstadoPago' =>'2',
			'nombre_inicial' => $archivo['name'],
			'nombre_archivo' => $archivoName,
			'extension' => FILES_WASABI[$tipoArchivo[1]],
