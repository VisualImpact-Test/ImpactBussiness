...
class M_EquiposComputo extends MY_Model
{
	var $resultado = [
		'query' => '',
		'estado' => false,
	...
	public function obtenerInformacionEquiposComputo($params = [])
	{
		$filtros = "";
		...
		$sql = "
			SELECT
				...
				, 'COTIZACION' AS tipoEquiposComputo
				, p.codEquiposComputo
				, c.idCuenta
				, c.nombre AS cuenta
			...
			LEFT JOIN compras.cotizacionEstado ce ON p.idEquiposComputoEstado = ce.idEquiposComputoEstado
	...
	public function obtenerInformacionEquiposComputoDetalle($params = [])
	{
		$filtros = "";
		$filtros .= !empty($params['idEquiposComputo']) ? ' AND p.idEquiposComputo = ' . $params['idEquiposComputo'] : '';
		...
		$sql = "
			...
			FROM compras.cotizacion p
			JOIN compras.cotizacionDetalle pd ON p.idEquiposComputo = pd.idEquiposComputo
			JOIN compras.itemTipo it ON pd.idItemTipo = it.idItemTipo
			JOIN compras.cotizacionEstado ce ON p.idEquiposComputoEstado = ce.idEquiposComputoEstado
			JOIN compras.cotizacionDetalleEstado cde ON pd.idEquiposComputoDetalleEstado = cde.idEquiposComputoDetalleEstado
			LEFT JOIN visualImpact.logistica.cuenta c ON p.idCuenta = c.idCuenta
			LEFT JOIN visualImpact.logistica.cuentaCentroCosto cc ON p.idCentroCosto = cc.idCuentaCentroCosto
	...
	public function validarExistenciaEquiposComputo($params = [])
	{
		$filtros = "";
		$filtros .= !empty($params['idEquiposComputo']) ? ' AND p.idEquiposComputo != ' . $params['idEquiposComputo'] : '';
	...
	public function insertarEquiposComputo($params = [])
	{
	...
	public function insertarEquiposComputoDetalle($params = [])
	{
	...
	public function actualizarEquiposComputo($params = [])
	{
