...
class M_ProveedorDocumento extends MY_Model
{
	...
	public function obtenerOCLibreEmail($params = [])
	{
		$this->db
			->distinct()
			->select("CASE WHEN mp.idMetodoPago = 1 THEN 'AL CONTADO' ELSE 'CRÉDITO' END AS metodoPago, mp.cantDias, CONVERT(VARCHAR(10), c.fechaAprobadoFinanza, 103) AS fechaAprobadoFinanza,
			DATENAME(month, c.fechaAprobadoFinanza) AS mesAprobacionFinanza,
