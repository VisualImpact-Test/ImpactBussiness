	...
	<fieldset style="margin-top:15px;margin-bottom:15px;">
		...
		<div>
					...
					<tbody>
						<?php foreach ($data as $key => $row) : ?>
							<tr class="default">
								...
								<td><a class="ui button" href="<?= RUTA_WASABI . 'validacionArte/' . verificarEmpty($row['nombre_archivo'], 3) ?>" target="_blank">Descargar</a></td>
							</tr>
						<?php endforeach; ?>
					</tbody>
				</table>
			...
			<div style="text-align: center;">
				<a class="boton verde" href="<?= $this->config->base_url() . 'FormularioProveedor/confirmarArte?pro='.base64_encode($proveedor['idProveedor']).'&cot='.base64_encode($cotizacion['idCotizacion']).'&ne='.base64_encode(1) ; ?>" target="_blank" rel="noopener noreferrer">Aprobar</a>
				<a class="boton rojo" href="<?= $this->config->base_url() . 'FormularioProveedor/confirmarArte?pro='.base64_encode($proveedor['idProveedor']).'&cot='.base64_encode($cotizacion['idCotizacion']).'&ne='.base64_encode(0) ; ?>" target="_blank" rel="noopener noreferrer">Rechazar</a>
			</div>
		</div>
	</fieldset>
</div>
