...
<?php foreach ($cotizacionDetalle as $k => $v) : ?>
	<?php if ($v['idItemTipo'] == COD_TEXTILES['id']) :  ?>
...
<table border="1" class="tb-detalle" style="width: 100%; margin-bottom: 100px;">
	<thead>
		<tr>
			...
			<?php if ($cotizacionDetalle[0]['idItemTipo'] == COD_SERVICIO['id']) :  ?>
	...
	<tbody>
		...
		<?php foreach ($cotizacionDetalle as $key => $row) : ?>
			...
			<?php if ($row['idItemTipo'] == COD_TEXTILES['id'] && !empty($detalleSubTalla[$row['idCotizacionDetalle']])) :  ?>
				<?php $first = true; ?>
				<?php foreach ($detalleSubTalla[$row['idCotizacionDetalle']] as $kcds => $vcds) : ?>
					<?php if ($first) :  ?>
						<?php $first = false; ?>
						<tr>
			...
			<?php else :  ?>
				<?php if ($row['idItemTipo'] == COD_SERVICIO['id']) :  ?>
					<?php $v1 = $cotizacionDetalleSub[$row['idCotizacionDetalle']][0]['sucursal'] ?>
					<?php $v2 = $cotizacionDetalleSub[$row['idCotizacionDetalle']][0]['razonSocial'] ?>
					<?php $v3 = $cotizacionDetalleSub[$row['idCotizacionDetalle']][0]['tipoElemento'] ?>
					<?php $v4 = $cotizacionDetalleSub[$row['idCotizacionDetalle']][0]['marca'] ?>
					<?php $costoTotal = 0; ?>
					<?php foreach ($cotizacionDetalleSub[$row['idCotizacionDetalle']] as $ks => $vs) : ?>
