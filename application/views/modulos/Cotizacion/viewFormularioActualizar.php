				...
				<div class="ui grid">
					<div class="sixteen wide tablet twelve wide computer column itemDet_1">
						...
						<div class="fields">
							...
							<div class="five wide field divCarProv">
						...
						<!-- Concurso -->
						<div class="ui grid d-none ml-0 div-features div-feature-<?= COD_CONCURSO['id'] ?>">
							<div class="row ml-0 pt-4">
							...
							<div class="three column row divDetalleConcurso">
								...
								<div class="column">
									<div class="ui sub header">Cantidad</div>
									<input class="cantidadSubItemConcurso keyUpChange onlyNumbers" name="cantidadSubItemConcurso[0]" placeholder="Cantidad" onchange="Cotizacion.calcularMontoTarjetasVales(this);">
								</div>
								<div class="column">
									<div class="ui sub header">Monto</div>
									<input class="montoSubItemConcurso keyUpChange onlyNumbers" name="montoSubItemConcurso[0]" placeholder="Monto" onchange="Cotizacion.calcularMontoTarjetasVales(this);">
								</div>
							</div>
						</div>
						<!-- Distribucion -->
										...
										<?php foreach ($cotizacionDetalleSub[$row['idCotizacionDetalle']][COD_TRANSPORTE['id']] as $dataSubItem) : ?>
											...
											<div class="body-sub-item body-sub-item-servicio" data-id="<?= $dataSubItem['idCotizacionDetalleSub'] ?>">
												...
												<div class="fields">
													<div class="five wide field">
														<div class="ui sub header">Tipo Móvil</div>
														<input class="inpTipoMovilTransporte formTransporte tipoMovilTransporte_transporte" name="tipoMovilTransporte[<?= $row['idCotizacionDetalle'] ?>]" value="<?= verificarEmpty($dataSubItem['tipo_movil'], 4) ?>" readonly>
													</div>
													<div class="three wide field">
														<div class="ui sub header">Csto Visual</div>
	...
	<div class="element-container">
		<a href="javascript:;">
			<?php if ($cotizacion['idCotizacionEstado'] <= ESTADO_CONFIRMADO_COMPRAS) : ?>
				<?php if ($btnEnviar) : ?>
					<span class="float-element tooltip-left btn-send" data-message="Enviar" onclick='Fn.showConfirm({ idForm: "formActualizarCotizacion", fn: "Cotizacion.actualizarCotizacionView(2)", content: "¿Está seguro de registrar y enviar esta cotizacion?" });'>
						<i class="send icon"></i>
					</span>
				<?php endif; ?>
				<span class="float-element tooltip-left btn-save" data-message="Guardar" onclick='Fn.showConfirm({ fn: "Cotizacion.actualizarCotizacionView()", content: "¿Está seguro de guardar esta cotizacion?" });'>
					<i class="save icon"></i>
				</span>
			...
			<?php endif; ?>
			<span class="float-element tooltip-left btn-return" onclick='Fn.showConfirm({ fn: "window.history.back();", content: "¿Está seguro de regresar a la pestaña anterior?" });' data-message="Regresar" style="background-color: red;">
