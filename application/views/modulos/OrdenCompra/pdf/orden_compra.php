	...
	<table border="1" style="width: 100%;">
		...
		<tbody style="border:1px solid black;">
			...
			<?php foreach ($dataOc as $k => $row) : ?>
				...
				$igv_total += ($row['csg_item'] * (!empty($dataOc[0]['igvPorcentaje']) ? (($dataOc[0]['igvPorcentaje'] + 100) / 100) : IGV));
				...
				<tr style="border-bottom: none;">
					...
					<td class="text-right">
						<?= !empty($row['costo_item']) ? monedaNew(['valor' => $row['costo_item'], 'simbolo' => $row['simboloMoneda'], 'cambio' => $row['monedaCambio']]) : 0 ?>
					</td>
					<td class="text-right">
						<?= !empty($row['csg_item']) ? monedaNew(['valor' => $row['csg_item'], 'simbolo' => $row['simboloMoneda'], 'cambio' => $row['monedaCambio']]) : 0 ?>
					</td>
				</tr>
		...
		<tfoot class="full-widtd">
			<tr class="height:100px">
				...
				<td class="text-right">
					<p><?= monedaNew(['valor' => $total, 'simbolo' => $dataOc[0]['simboloMoneda'], 'cambio' => $row['monedaCambio']]) ?></p>
					<p><?= monedaNew(['valor' => $igv_total, 'simbolo' => $dataOc[0]['simboloMoneda'], 'cambio' => $row['monedaCambio']]) ?></p>
					<p><?= monedaNew(['valor' => $igv_total + $total, 'simbolo' => $dataOc[0]['simboloMoneda'], 'cambio' => $row['monedaCambio']])  ?></p>
				</td>
			</tr>
			<tr>
				<td colspan="6" class="text-left">
					Son: <?= moneyToText(['numero' => ($igv_total + $total)/$monedaCambio, 'moneda' => $dataOc[0]['monedaPlural']]) ?>
				</td>
			</tr>
